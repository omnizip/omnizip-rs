...
mod tests {
    ...
    fn empty_tree_lookup() {
        ...
        assert_eq!(depth, 0);
        assert!(node.lookup(b'a').is_none());
    }
    ...
    fn add_and_lookup() {
        ...
        t.add_symbol(b"th", b'e');
        t.add_symbol(b"th", b'e');
        t.add_symbol(b"th", b'a');

        let (node, _depth) = t.walk(b"th");
        let (lo, freq, total) = node.lookup(b'e').expect("e should be present");
        assert_eq!(freq, 2);
        assert_eq!(lo, 0); // 'e' is first-seen, so cum=0
        assert_eq!(total, 3);
        let (lo_a, freq_a, _) = node.lookup(b'a').expect("a should be present");
        assert_eq!(freq_a, 1);
        assert_eq!(lo_a, 2); // after 'e' (freq 2)
    }
    ...
    fn escape_slot_nonzero() {
        ...
        let (node, _) = t.walk(b"x");
        let (lo, width) = node.escape_slot();
        assert_eq!(lo, 1); // total
        assert_eq!(width, 2); // 1 distinct + 1
        assert!(width > 0);
        assert!(node.coding_total() > node.total);
    }
    ...
    fn order_capped() {
        ...
        // the deepest context — that's the suffix ending at the current
        // position, which is standard PPM semantics. So walking "de"
        // should reach depth 2; walking the full "abcde" stops at depth 0
        // because "a" was never added as a context root.
        t.add_symbol(b"abcde", b'f');
        let (_node, depth_de) = t.walk(b"de");
        assert_eq!(depth_de, 2);
        let (_node, depth_full) = t.walk(b"abcde");
        assert_eq!(depth_full, 0);
    }
}

...
const fn build_cmd_lut() -> [CmdLutElement; 704] {
    ...
    while i < 23 {
        ...
        copy_length_offsets[i + 1] = copy_length_offsets[i] + (1u16 << K_COPY_LENGTH_EXTRA_BITS[i]);
        i += 1;
    }
    ...
    while symbol < 704 {
        ...
        let context: u8 = if copy_len_offset > 4 {
            3
        } else {

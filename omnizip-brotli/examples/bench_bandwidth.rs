...
fn ref_run(args: &[&str], infile: &std::path::PathBuf, expected: Option<&[u8]>) -> Option<(Vec<u8>, f64)> {
...
fn run(label: &str, data: &[u8], levels: &[i32]) {
    ...
    for &q in levels {
        ...
        let (refc, enc_secs) =
            match ref_run(&["-q", &q.to_string(), "-c"], &tmp, None) {
                Some(r) => r,
                None => {
        ...
        let (_, dec_secs) = ref_run(&["-d", "-c"], &br_path, Some(data)).unwrap_or((vec![], f64::NAN));
        ...
        let verify = if rt_ok && ref_rt_ok && interop { "OK" } else { "FAIL" };
        println!(
